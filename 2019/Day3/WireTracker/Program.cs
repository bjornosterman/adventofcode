...
namespace WireTracker
{
    class Program
    {
        static void Main(string[] args)
        {
            //var input1 = "R75,D30,R83,U83,L12,D49,R71,U7,L72".Split(",");
            //var visitedPoints1 = Run(input1);
            //var input2 = "U62,R66,U55,R34,D71,R55,D58,R83".Split(",");
            //var visitedPoints2 = Run(input2);

            var (visitedPoints1, lengths1) = Run(PuzzleInputs.Input1.Split(","));
            var (visitedPoints2, lengths2) = Run(PuzzleInputs.Input2.Split(","));

            var intersect = visitedPoints1.Intersect(visitedPoints2);
            var shortest = intersect
                .Select(x => lengths1[x]+lengths2[x])
                .Where(x => x > 0)
                .Min();
            Console.WriteLine("Smallest = " + shortest);
        }

        static (HashSet<Point>, Dictionary<Point, int>) Run(string[] steps)
        {
            var length = 0;
            var point = new Point(0, 0);
            var visitedPoints = new HashSet<Point>();
            var lengths = new Dictionary<Point, int>();

            foreach (var step in steps)
            {
                var transformer = GetTransformer(step[0]);
                ...
                for (int i = 0; i < stepLength; i++)
                {
                    ...
                    {
                        lengths.Add(point, length);
                    }
                }
            }
            return (visitedPoints, lengths);
        }

        private static Func<Point, Point> GetTransformer(char x)
        {
            switch (x)
            {
                case 'R': return p => new Point(p.X + 1, p.Y);
                case 'U': return p => new Point(p.X, p.Y + 1);
                case 'L': return p => new Point(p.X - 1, p.Y);
                case 'D': return p => new Point(p.X, p.Y - 1);
                default:
                    throw new Exception();
            }
        }
    ...
    public struct Point
    {
        ...
        {
            return this.Y.GetHashCode() * 17 + this.Y.GetHashCode();
        }
