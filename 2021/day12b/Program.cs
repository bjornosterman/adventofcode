...
namespace day {
    class Program {
        ...
        static void Traverse(string location, List<string> visited, bool used) {
            ...
            if (location == "end") {
                ...
                foreach (var adj in paths[location]) {
                    if (adj != "start") {
                        if ((new_visited.Contains(adj) == false || !used) || adj[0] < 'a') {
                            Traverse(adj, new_visited, used || (adj[0] >= 'a' && new_visited.Contains(adj)));
                        }
                    }
                }
            }
